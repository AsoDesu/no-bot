...
class RemoveReactionRole extends BaseCommand {
	async execute(msg: Message, args: string[]) {
		...
		if (args.length < 2) {
			msg.channel.send("Usage: >removerr (msgid) (emote)");
			return;
		}
		...
		if (!data.exists) {
			msg.channel.send("That message isn't linked to any reaction roles.");
			return;
		}
		...
		if (!message) {
			msg.channel.send("Couldn't find the specified message.");
			return;
		}

		if (args[1] == "all") {
			message.reactions.removeAll();
			db.collection("reactions").doc(msg.guild.id).collection("reactions").doc(message.id).delete();
		} else {
			var emote: string;
			if (args[1].includes("<")) {
			...
			if (emoteIndex == null) {
				msg.channel.send("That emote doesn't exist on that message.");
				return;
			}
...
commandManager.registerCommand(new RemoveReactionRole());
